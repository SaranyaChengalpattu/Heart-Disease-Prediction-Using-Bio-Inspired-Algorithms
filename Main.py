...
def geneticAlgorithm():
    global classifier, ga_acc
    text.delete('1.0', END)
    global ga_acc
    train = pd.read_csv(filename)

    test = pd.read_csv('heart_dataset.txt')
...
def predict():
    ...
    if 'classifier' not in globals():
        text.insert(END, "Please run one of the algorithms (Genetic Algorithm, BAT Algorithm, or BEE Algorithm) first.\n")
        return
    text.delete('1.0', END)
